...
type MouseFollower1Props = {
  children: ReactNode;
};

// const IMAGES = [
//   "/mouse-follower/image1.png",
//   "https://cdn.cosmos.so/6a854a1b-5c06-45b1-b055-4a4652ba4e21?format=jpeg",
//   "https://cdn.cosmos.so/3c35a1b1-717b-4219-9282-881a762724f2?format=jpeg",
//   "https://cdn.cosmos.so/8a6998b4-fce7-48c4-b40c-9b90bcf0007c?format=jpeg",
//   "https://cdn.cosmos.so/f798acc8-6bc8-4f2c-ace2-2440f2be4795?format=jpeg",
...
// ];
const IMAGES = [
  "/mouse-follower/image1.jpg",
  "/mouse-follower/image2.jpg",
  "/mouse-follower/image3.jpg",
  "https://cdn.cosmos.so/15a7b84c-ba74-470f-8813-25eb0a0d8ba2?format=jpeg",
    "https://cdn.cosmos.so/6a854a1b-5c06-45b1-b055-4a4652ba4e21?format=jpeg",
  "https://cdn.cosmos.so/3c35a1b1-717b-4219-9282-881a762724f2?format=jpeg",
  "https://cdn.cosmos.so/8a6998b4-fce7-48c4-b40c-9b90bcf0007c?format=jpeg",
...
const MouseFollower1 = ({ children }: MouseFollower1Props) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const flairRefs = useRef<(HTMLImageElement | null)[]>([]);
  const mousePosRef = useRef<MousePos>({ x: 0, y: 0 });
  const lastMousePosRef = useRef<MousePos>({ x: 0, y: 0 });
  ...
  useEffect(() => {
    ...
    const playAnimation = (img: HTMLImageElement) => {
      gsap
        .timeline()
        .from(img, {
          opacity: 0,
    ...
    const animateImage = () => {
      ...
      const index = wrapIndex(indexRef.current);
      const img = flairRefs.current[index];
      if (!img) return;
      ...
      // const y = Math.max(bounds.top, Math.min(mousePosRef.current.y, bounds.bottom));

      const Y_OFFSET = 40; // adjust this value

      const y = Math.max(
        bounds.top,
        Math.min(mousePosRef.current.y - Y_OFFSET, bounds.bottom),
      );
      gsap.set(img, {
        opacity: 1,
        left: x,
