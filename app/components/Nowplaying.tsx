...
export default function MusicStatus() {
  ...
  const [resolvedArt, setResolvedArt] = useState<string | null>(null); // ✅ moved to top
  ...
  // ✅ Also moved to top — always runs regardless of track state
  useEffect(() => {
    if (!track?.title || !track?.artist) return;

    // Use albumArt from API if available
    if (track.albumArt) {
      setResolvedArt(track.albumArt);
      return;
    }
              ...
              {track.playing ? (
                <div className="flex items-center gap-2 text-sm text-green-500 font-medium">
                  ...
                  <div className="flex gap-[2px] ml-1 items-end">
                    <span className="w-[3px] h-3 bg-green-500 animate-bounce"></span>
                    <span className="w-[3px] h-4 bg-green-500 animate-bounce [animation-delay:100ms]"></span>
                    <span className="w-[3px] h-2 bg-green-500 animate-bounce [animation-delay:200ms]"></span>
                  </div>
                </div>
              ) : (
